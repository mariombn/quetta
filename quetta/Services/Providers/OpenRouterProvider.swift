...
//  quetta
//
//  OpenRouter provider (API key). Uses the OpenAI-compatible Chat Completions
//  endpoint with the required HTTP-Referer and X-Title headers.
//

import Foundation

nonisolated final class OpenRouterProvider: TranslationProvider, SummaryProvider, @unchecked Sendable {
    ...
    private func addHeaders(to request: inout URLRequest, contentType: Bool = false) {
        ...
        request.setValue("https://quetta.app", forHTTPHeaderField: "HTTP-Referer")
        request.setValue("Quetta", forHTTPHeaderField: "X-Title")
        if contentType {
